...
public class WorldRenderer {
	...
	public void render () {
		if (world.bj.position.y > cam.position.y) cam.position.y = world.bj.position.y;
		cam.update();
	...
	private void renderBob () {
		...
		switch (world.bj.state) {
		case Bj.BOB_STATE_FALL:
			keyFrame = Assets.bobFall.getKeyFrame(world.bj.stateTime, Animation.ANIMATION_LOOPING);
			break;
		case Bj.BOB_STATE_JUMP:
			keyFrame = Assets.bobJump.getKeyFrame(world.bj.stateTime, Animation.ANIMATION_LOOPING);
			break;
		case Bj.BOB_STATE_HIT:
		default:
			keyFrame = Assets.bobHit;
		}

		float side = world.bj.velocity.x < 0 ? -1 : 1;
		if (side < 0)
			batch.draw(keyFrame, world.bj.position.x + 0.5f, world.bj.position.y - 0.5f, side * 1, 1);
		else
			batch.draw(keyFrame, world.bj.position.x - 0.5f, world.bj.position.y - 0.5f, side * 1, 1);
	}
	...
	private void renderItems () {
		...
		for (int i = 0; i < len; i++) {
			...
			TextureRegion keyFrame = Assets.fireAnim.getKeyFrame(coin.stateTime, Animation.ANIMATION_LOOPING);
